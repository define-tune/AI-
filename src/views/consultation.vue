        ...
        <div class="sidebar">
            ...
            <div class="ai-assistant-info">
                <div class="breathing-circle">
                 ...
                 <h3 class="assistant-name">ChatGPT</h3>
                    ...
                    <div class="session-item" v-for="item in sessionList" :key="item.id" @click="handleSessionClick(item)">
                        <div class="session-info">
                            ...
                            <div class="session-actions">
                                <el-button text type="danger" @click="handleDeleteSession(item.id)">
                                    <el-icon>
                                        <DeleteFilled/>
                                    </el-icon>
                                </el-button>
             ...
             <div class="chat-messages">
                ...
                <div v-if="messages.length === 0" class="message-item ai-message">
                    ...
                    <div class="message-content">
                        <div class="message-bubble">
                            <p>您好！我是GPT，您的AI心理健康助手，很高兴陪伴您，为您提供温暖的心理支持。请告诉我，今天您感觉怎么样？有什么想要分享的吗？</p>
                        </div>
                ...
                <!-- 消息列表 -->
                 <div class="message-item" v-for="item in messages" :key="item.id" :class="item.senderType === 1 ? 'user-message' : 'ai-message'">
                    <div class="message-avatar">
                        <el-image v-if="item.senderType === 1" :src="iconUrl2" style="width: 18px; height: 18px;"></el-image>
                        <el-image v-if="item.senderType === 2" :src="iconUrl" style="width: 18px; height: 18px;"></el-image>
                    </div>
                    <div class="message-content">
                        <div class="message-bubble">
                            <!-- ai正在思考中 -->
                            ...
                            <!-- ai 错误提示 -->
                             ...
                             <!-- ai正常返回消息 -->
                              <MarkdownRenderer v-else-if="item.senderType === 2 && !item.isError" :content="item.content" :isAiMessage="true"></MarkdownRenderer>
                              <p v-else-if="item.content" v-html="formatMessageContent(item.content)"></p>
                        </div>
                        <div class="message-time">{{ item.senderType === 2 && isAiTyping ? '正在输入中...' : item.createdAt }}</div>
                    </div>
                 </div>
             </div>
             <!-- 消息输入区域 -->
              <div class="chat-input">
                ...
                <el-button :disabled="!userMessage.trim()|| userMessage.length > 500" class="send-btn" type="primary" @click="sendMsg">
                    <el-icon>
                        <Promotion/>
                    </el-icon>
                </el-button>
...
const createNewFrontendSession = () => {
    ...
    currentSesson.value = newSession
    messages.value = []
}
//定义一个当前会话对象
const currentSesson = ref(null)
...
const sendMsg = () => {
    ...
    //如果没有会话，或者临时会话，那么需要创建新的会话
    if(currentSesson.value.status === 'TEMP') {
        ...
        startAIResponse(currentSesson.value.sessionId, message)
    }
}
...
const startNewSession = (message) => {
    ...
    if(currentSesson.value.sessionTitle === '新对话') {
        sessionParams.sessionTitle = `GPT-4.5 - ${new Date().toLocaleString()}`
    } else {
        //如果是历史会话记录
        sessionParams.sessionTitle = currentSesson.value.sessionTitle
    }
    startSessionAPI(sessionParams).then(res => {
        ...
        //如果当前是临时会话，需要更新
        if(currentSesson.value && currentSesson.value.status === 'TEMP') {
            //更新为正式会话
            Object.assign(currentSesson.value, sessionData)
        } else {
            currentSesson.value = sessionData
        }
        //更新会话列表
        getSessionList(currentSesson.value.sessionId, message)
        //添加初始用户消息
        messages.value.push({
            id: Date.now(),
            senderType: 1,
